	...
	mod_add = signal_generator(user_input);
 	...
 	// Read sensors, model signals, select between them, synthesize injection
  	sense_synth_select(...) {
		...
		// Sim calculation
		Sen->Vb_model(Sen->Sim->calculate()){							// PV to Vb_model_
			// Inputs
  			Tb_ = Sen->Tb(){
				return Tb_;											// FV
			}
			...
			dt_charge_ = dt_fut_;										// Feedback --> PV source
			ib_charge_ = ib_fut_;  // Same time plane as volt, added past value	// Feedback --> PV source
			return vb_;												// PV 
		} // Sen->Sim->calculate()
