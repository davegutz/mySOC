...
def compare_run_sim(
    ...
    if True:
        ...
        if filename_root is None:
            ...
            for obj, struct_name in (
                ...
                (mon_ver, "mon_ver"),
                (sim_run, "sim_run"),
                (sim_s_ver if sim_s_ver is not None else sim_ver, "sim_ver"),
            ):
    ...
    if plots:
        if data_file_clean is not None:
            dir_root_test, data_root_test = (
                str(PurePosixPath(data_file_clean).parent),
                PurePosixPath(data_file_clean).name,
            )
            data_root_test = data_root_test.replace(".csv", "")
            aug_file = (
                PurePosixPath(data_file_clean).name.replace(".csv", "_") + PurePosixPath(Path(__file__).as_posix()).stem
            )
        else:
            dir_root_test, data_root_test = (
                str(PurePosixPath(temp_flt_file_clean).parent),
                PurePosixPath(temp_flt_file_clean).name,
            )
            data_root_test = data_root_test.replace(".csv", "")
            aug_file = (
                PurePosixPath(temp_flt_file_clean).name.replace(".csv", "_")
    ...
    if compare_run_ver:
        try:
            from CompareRunVer import (
                temp_folder,
                find_pairs,
                compare_pair,
                report as ver_report,
                plot_diffs as ver_plot_diffs,
            )
            ...
            if _ver_version and _case_stem:
                ...
                if Path(_ver_temp).is_dir():
                    ...
                    _ver_pairs = [
                        (r, v)
                        for r, v in _ver_pairs
                        if ("_mon_run" in r.name or "_sim_run" in r.name) and r.name.startswith(_case_stem)
                    ]
                    if _ver_pairs:
                        _tol = 1e-3
                        _rtol = (
                            1e-3  # ~3 significant digits — keeps q_capacity-class large-magnitude signals from tripping
                        )
...
def main():  # Example usage.  ok on 20260217
    ...
    """Request history:
        ...
        9:  dyn_n
    """
    data_file = '/home/daveg/.local/SOC_Particle/dataReduction/g20260612a/ampHiEmptFail_soc3p2_hi_lo_bb.csv'
