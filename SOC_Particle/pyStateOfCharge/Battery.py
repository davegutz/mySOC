...
from Faults import Diff, Looparound, Wrap
...
class Battery(BatteryConstants, Coulombs):
    ...
    def __init__(
        ...
        Coulombs.__init__(
            ...
            q_cap_rated_scaled=q_cap_rated,
            t_rated=t_rated,
        ...
        self.dv_dyn = 0.0  # Model current induced back emf, V
        self.ib_dyn = 0.0  # Model current induced back emf before resistance multiply, A
        self.ib_dyn_T = 0.0  # Randles update time, s
        self.ib_dyn_rstate = 0.0  # Randles rstate, A
        self.ib_dyn_lstate = 0.0  # Randles lstate, A
        self.vb = Battery.NOMINAL_VB  # Battery voltage at post, V
        self.ib = 0.0  # Current into battery post, A
        self.ib_in = 0.0  # Current into calculate, A
        self.ib_charge = 0.0  # Current into count_coulombs, A
        self.ioc = 0  # Current into battery process accounting for hysteresis, A
        self.dv_dsoc = 0.0  # Slope of soc-voc curve, V/%
        self.tcharge = 0.0  # Charging time to 100%, hr
        ...
        self.dv_hys = 0.0  # Placeholder so BatterySim can be plotted
        self.tau_hys = 0.0  # Placeholder so BatterySim can be plotted
        self.dv_dyn = 0.0  # Placeholder so BatterySim can be plotted
        self.ib_dyn = 0.0  # Placeholder so BatterySim can be plotted
        self.ib_dyn_T = 0.0  # Placeholder so BatterySim can be plotted
        self.ib_dyn_rstate = 0.0  # Placeholder so BatterySim can be plotted
        self.ib_dyn_lstate = 0.0  # Placeholder so BatterySim can be plotted
        self.bms_off = False
        ...
        self.Tb_state = Tb_f
        self.Tb_hdwe_f = Tb_f
        self.Tb_hdwe_f_rate = 0.0
        self.Tb_model_f_rate = 0.0
        self.Tb_rstate = None
        self.Tb_state = Tb_f
        self.Tb_hdwe_f = Tb_f
        self.Tb_hdwe_f_rate = 0.0
    ...
    def __str__(self, prefix=""):
        ...
        s += (
            "  r_sd    = {:9.6f}  // Equivalent model for EKF reference.	Parasitic discharge equivalent, ohms\n"
        ).format(self.chemistry.r_sd)
        s += (
            "  tau_sd  = {:9.1f}  // Equivalent model for EKF reference.	Parasitic discharge time constant, sec\n"
    ...
    def calc_h_jacobian(self, soc_lim, tb_f):
        if soc_lim > 0.5:
            dv_dsoc = (
                self.chemistry.lookup_voc(soc_lim, tb_f) - self.chemistry.lookup_voc(soc_lim - 0.01, tb_f)
            ) / 0.01
        else:
            dv_dsoc = (
                self.chemistry.lookup_voc(soc_lim + 0.01, tb_f) - self.chemistry.lookup_voc(soc_lim, tb_f)
            ) / 0.01
        return dv_dsoc
...
class BatteryMonitor(Battery, EKF1x1, Wrap):
    ...
    def __init__(
        ...
        OPT=None,
        SN=None,
        q_cap_rated=Battery.NOM_UNIT_CAP * 3600,
        t_rated=Battery.RATED_TEMP,
        temp_rlim=0.017,
        scale=1.0,
        Tb_f=Battery.NOMINAL_TB,
        tweak_test=False,
        dvoc=0.0,
        mod_code=0,
        vsat_add=0.0,
        ...
        self.tcharge_ekf = 0.0  # Charging time to 100% from ekf, hr
        self.voc = 0.0  # Charging voltage, V
        ...
        self.e_soc_ekf = 0.0  # analysis parameter
        self.e_voc_ekf = 0.0  # analysis parameter
        ...
        self.soc_s = 0.0  # Model information
        self.EKF_converged = TFDelay(False, Battery.EKF_T_CONV, Battery.EKF_T_RES, Battery.EKF_NOM_DT)
        self.voc_stat_filt = LagExp(
            self.EKF_NOM_DT, self.VOC_STAT_FILT, self.VB_MIN, self.VB_MAX
        ...
        self.ChargeTransfer = LagExp(
            ...
            max_=Battery.NOM_UNIT_CAP * scale,
            min_=-Battery.NOM_UNIT_CAP * scale,
            tau=self.chemistry.tau_ct,
        )
        self.ib = 0.0
        self.vb = 0.0
        self.vb_model_rev = 0.0
        self.voc_stat = 0.0
        self.voc_stat_f = 0.0
        self.voc = 0.0
        self.voc_dead = 0.0
        ...
        self.ib_dyn_r = False
        self.ib_dyn_T = 0.0
        self.ib_dyn_rstate = 0.0
        self.ib_dyn_lstate = 0.0
        self.voc_stat_f_rstate = 0.0
        self.voc_stat_f_lstate = 0.0
        self.voc_stat_f_tau = 0.0
        self.voc_stat_f_T = 0.0
        self.voc_ekf = 0.0
        self.eframe = 0
        if OPT is not None:
        ...
        self.voc_stat_s = 0.0
        self.dv_dyn_s = 0.0
        self.d_delta_q_s = 0.0
        self.delta_q_s = 0.0
        self.dv_hys_s = 0.0
        self.ib_charge_s = 0.0
        self.ib_dyn_s = 0.0
        self.ib_in_s = 0.0
        self.ib_s = 0.0
        self.ioc_s = 0.0
    ...
    def calculate(
        ...
        self.wrap(
            reset=reset,
            ib_noa_hdwe=self.ib_noa_hdwe,
            SN=SN,
            ib_amp=self.ib_amp,
            ib_noa=self.ib_noa,
            ib_amp_pst=self.ib_amp_pst,
        ...
        if self.reset:
            ...
            if not self.bms_off:
                ...
                if (self.voltage_low != voltage_low_past) and not self.reset:
                    print(
                        f"\nBMS OFF voc_stat {self.voc_stat} vb_down {self.chemistry.vb_down} "
                ...
                if self.voltage_low != voltage_low_past:
                    print(
                        f"\nBMS ON voc_stat {self.voc_stat} vb_down {self.chemistry.vb_down} "
                        f"vb_rising {self.chemistry.vb_rising} bms_off {self.bms_off} "
                        f"voltage_low {self.voltage_low} \n\n"
                    )
        ...
        if calc_ekf:
            ...
            if ddq_dt > 0.0 and not self.tweak_test:
        ...
        if self.ib_charge > 0.1:
            self.tcharge_ekf = min(Battery.NOM_UNIT_CAP / self.ib_charge * (1.0 - self.soc_ekf), 24.0)
        elif self.ib_charge < -0.1:
            self.tcharge_ekf = max(Battery.NOM_UNIT_CAP / self.ib_charge * self.soc_ekf, -24.0)
        elif self.ib_charge >= 0.0:
            self.tcharge_ekf = 24.0 * (1.0 - self.soc_ekf)
        else:
            self.tcharge_ekf = -24.0 * self.soc_ekf
    ...
    def calc_charge_time(self, q, q_capacity, charge_curr, soc):
        ...
        if charge_curr > Battery.TCHARGE_DISPLAY_DEADBAND:
            self.tcharge = min(-delta_q / charge_curr / 3600.0, 24.0)
        elif charge_curr < -Battery.TCHARGE_DISPLAY_DEADBAND:
            self.tcharge = max((q_capacity + delta_q - self.q_min) / charge_curr / 3600.0, -24.0)
        elif charge_curr >= 0.0:
            self.tcharge = 24.0
        else:
            self.tcharge = -24.0

        amp_hrs_remaining = (q_capacity - self.q_min + delta_q) / 3600.0
        if soc > self.soc_min:
            ...
            self.amp_hrs_remaining_ekf = 0.0
            self.amp_hrs_remaining_wt = 0.0
        return self.tcharge
    ...
    def ekf_predict(self):
        """Process model"""
        self.Fx = 1.0 - self.dt_eframe / self.chemistry.tau_sd
...
class BatterySim(Battery):
    ...
    def __init__(
        ...
        self.hys = Hysteresis(
            scale=OPT.slr_hys_sim * Battery.ap_hys_scale,
            dv_hys=OPT.sim_run.dv_hys_s[0],
            scale_cap=OPT.slr_hys_cap_sim,
            slr_cap_chg=OPT.slr_cap_chg,
            slr_cap_dis=OPT.slr_cap_dis,
            slr_hys_chg=OPT.slr_hys_chg,
            slr_hys_dis=OPT.slr_hys_dis,
            chem=self.chem,
    ...
    def count_coulombs(self, OPT, SN, chem, reset_temp, tb_f, charge_curr, sat, saturated, mon_sat=None, rp=None, mon_delta_q=None):
        ...
        if (rp.modeling_ib and not self.reset_temp_past) or (not rp.modeling_ib and not reset_temp):
            ...
            self.delta_q = max(min(self.delta_q, 0.0), -self.q_capacity * 1.2)
        self.q = self.q_capacity + self.delta_q

        # Normalize
        if self.q_capacity != 0.0 and self.q_capacity is not None:
...
def sat_voc(tb_f, rated_temp, vsat, dvoc_dt, vsat_add=0.0):
