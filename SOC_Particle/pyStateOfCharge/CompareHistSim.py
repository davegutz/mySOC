# CompareHistSim.py: Load fault, history, and summary data dumps and compare to simulation
# Copyright (C) 2026 Dave Gutz
#
# noinspection
# PyPep8Naming,PyUnboundLocalVariable,PyShadowingNames,PyShadowingBuiltins,PyUnresolvedReferences,PyAttributeOutsideInit
# type: ignore
# pylint: disable=invalid-name, used-before-assignment, redefined-outer-name, redefined-builtin, no-member,
...
"""Load fault, history, and summary data dumps (Hist vs Sim), resample data,
evaluate simulation performance, and generate comparison plots."""
...
IB_BAND = 1.0  # Threshold to declare charging or discharging
TB_BAND = 25.0  # Band around temperature to group data and correct.  Large value means no banding, effectively
...
def fault_thr_bb(Tb, soc, voc_soc, voc_stat, C_rate, bb):
    ...
    WRAP_HI_A = 32.0  # Wrap high voltage threshold, A (32 after testing; 16=0.2v)
    WRAP_LO_A = -32.0  # Wrap high voltage threshold, A (-32, -20 too small on truck -16=-0.2v)
    ...
    IBATT_DISAGREE_THRESH = 10.0  # Signal selection threshold for current disagree test, A (10.)
    QUIET_A = 0.005  # Quiet set threshold, sec (0.005, 0.01 too large in truck)
    WRAP_SOC_HI_OFF = 0.97  # Disable e_wrap_hi when saturated
    WRAP_SOC_HI_SCLR = 1000.0  # Huge to disable e_wrap
    ...
    WRAP_SOC_LO_SCLR = 60.0  # Large to disable e_wrap (60. for startup)
    CC_DIFF_SOC_DIS_THRESH = 0.2  # Signal selection threshold for Coulomb counter EKF disagree test (0.2)
    CC_DIFF_LO_SOC_SCLR = 4.0  # Large to disable cc_ctf
    WRAP_MOD_C_RATE = 0.05  # Moderate charge rate threshold to engage wrap threshold 0
    WRAP_SOC_MOD_OFF = 0.85  # Disable e_wrap_lo when nearing saturated and moderate C_rate (0.85)

    vsat_ = bb.chemistry.nom_vsat + (Tb - 25.0) * bb.chemistry.dvoc_dt
    ...
    if soc >= WRAP_SOC_HI_OFF:
        ...
        ewsat_sclr_ = 1.0
        ewmin_sclr_ = 1.0
    ewhi_sclr_ = 1.0  # ram adjusts during data collection
    ewhi_thr = bb.chemistry.r_ss * WRAP_HI_A * ewhi_sclr_ * ewsat_sclr_ * ewmin_sclr_
    ewlo_sclr_ = 1.0  # ram adjusts during data collection
    ewlo_thr = bb.chemistry.r_ss * WRAP_LO_A * ewlo_sclr_ * ewsat_sclr_ * ewmin_sclr_

    # ib_diff
    ib_diff_sclr_ = 1.0  # ram adjusts during data collection
    ib_diff_thr = IBATT_DISAGREE_THRESH * ib_diff_sclr_

    # ib_quiet
    ib_quiet_sclr_ = 1.0  # ram adjusts during data collection
...
def bandaid(h):
    ...
    chm_s = h["chm_s"].copy()
    dt_s = h["dt"].copy()
    dv_dyn_s = h["dt"].copy() * 0.0
    dv_hys_s = h["dt"].copy() * 0.0
    ...
    mon_run = rf.rec_append_fields(mon_run, "reset", res)
    ...
    mon_run = rf.rec_append_fields(mon_run, "reset_all_faults", np.zeros(len(h.time_ux), dtype=bool))
    ...
    sim_run = rf.rec_append_fields(sim_run, "cutback_s", sat_s * 0.0)
    sim_run = rf.rec_append_fields(sim_run, "soc_s", soc_s)
    sim_run = rf.rec_append_fields(sim_run, "Tb_s", Tb_s)
    sim_run = rf.rec_append_fields(sim_run, "Tb_f_s", Tb_f_s)
    sim_run = rf.rec_append_fields(sim_run, "ib_s", ib_in_s)
    sim_run = rf.rec_append_fields(sim_run, "ib_dyn_s", ib_in_s)
    sim_run = rf.rec_append_fields(sim_run, "ib_charge_s", ib_in_s)
    sim_run = rf.rec_append_fields(sim_run, "ib_in_s", ib_in_s)
    sim_run = rf.rec_append_fields(sim_run, "ioc_s", ib_in_s)
...
def filter_Tb(raw, tb_forr, mon, tb_band=5.0, rated_batt_cap=None):
    ...
    for i in range(len(h.Tb_f)):
        saturated_[i] = is_sat(
            h.Tb_f[i],
            mon.chemistry.rated_temp,
            h.voc_f[i],
            h.soc[i],
            mon.chemistry.nom_vsat,
            mon.chemistry.dvoc_dt,
            mon.chemistry.low_t,
        )
        bms_off_[i] = (h.Tb_f[i] < mon.chemistry.low_t) or (
            (h.voc_stat_f[i] < 10.5) and (h.ib_f[i] < Battery.IB_MIN_UP)
        )

    # Correct for temp
    q_cap = calculate_capacity(
        q_cap_rated_scaled=rated_batt_cap * 3600.0,
        dqdt=mon.chemistry.dqdt,
        tb_f=h.Tb_f,
        t_rated=mon.chemistry.rated_temp,
    )
    dq = (h.soc - 1.0) * q_cap
    dq -= mon.chemistry.dqdt * q_cap * (tb_forr - h.Tb_f)
    q_cap_r = calculate_capacity(
        q_cap_rated_scaled=rated_batt_cap * 3600.0,
        dqdt=mon.chemistry.dqdt,
        tb_f=tb_forr,
        t_rated=mon.chemistry.rated_temp,
    )
    soc_r = 1.0 + dq / q_cap_r
    ...
    if len(h.time_ux) > 1:
        ...
        dt_hys_min = 1.0  # ??????????????????????????????
        print(f" {t_s_min=} {t_e_min=} {dt_hys_min=}  days of data = {(t_e_min - t_s_min) / (24.0 * 60)} ", end="")
...
def shift_time(mr, extra_shift=0.0):
    # Shift time
    first_non_zero = 0
    n = len(mr.time)
    ...
    if first_non_zero < n:  # success
        if first_non_zero > 0:
            shift = (mr.time[first_non_zero] + mr.time[first_non_zero - 1]) / 2.0
        else:
...
def add_delta_q(hist):
    delta_q = []
    for i in range(len(hist.time_ux)):
        q_capacity = hist.qcrs[i] * (1.0 + hist.dqdt[i] * (hist.Tb_f[i] - Battery.RATED_TEMP))
        delta_q.append(-q_capacity * (1.0 - hist.soc[i]))
...
def load_hist_and_prep(
    ...
    battery_file_clean = write_clean_file(
        data_file, type_="_battery", hdr_key=battery_hdr, unit_key=battery_val, skip=skip
    )
    ...
    # # Load mon v4 (old)
    if use_mon_csv:
        mon, sim, fault, mon_t_file_clean, temp_mont_t_file_clean, _ = load_data(
            data_file, 1, unit_key=unit_key, time_end=time_end, zero_zero=False, mon_str="hist"
        )
        mon = rename_all(mon)
    else:
        mon = None
    ...
    # Load summaries
    s_raw = None
    temp_sum_file_clean = write_clean_file(
        data_file, type_="_flt", hdr_key="fltb", unit_key="unit_u", skip=1, comment_str="---"
    )
    ...
    # Load history
    h_raw = None
    temp_hist_file_clean = write_clean_file(
        data_file, type_="_flt", hdr_key="fltb", unit_key="unit_h", skip=1, comment_str="---"
    )
    ...
    # Load fault
    temp_flt_file_clean = write_clean_file(
        data_file, type_="_flt", hdr_key="fltb", unit_key="unit_f", skip=1, comment_str="---"
    )
    ...
    if temp_flt_file_clean is not None:
        filename = (
            PurePosixPath(temp_flt_file_clean).name.replace(".csv", "_") + PurePosixPath(Path(__file__).as_posix()).stem
        )
    elif temp_hist_file_clean is not None:
        filename = (
            PurePosixPath(temp_hist_file_clean).name.replace(".csv", "_")
            + PurePosixPath(Path(__file__).as_posix()).stem
        )
    elif temp_sum_file_clean is not None:
        filename = (
            PurePosixPath(temp_sum_file_clean).name.replace(".csv", "_") + PurePosixPath(Path(__file__).as_posix()).stem
        )
    ...
    if use_mon_csv is True and mon is not None:
        if Battery.CHEM != int(mon.chm[0]):
            print(
                f"CompareHistSim::WARNING  Battery.CHEM {Battery.CHEM} not equal to "
                f"transient input chm {int(mon.chm[0])}"
            )
    ...
    if f_raw is not None:
        ...
        if len(f_raw) > 0:
            ...
            fault = filter_Tb(
                fault, 20.0, batt, tb_band=100.0, rated_batt_cap=rated_batt_cap
            )  # tb_band=100 disables banding
        else:
            fault = None
    ...
    if h_combo_raw is None or np.atleast_1d(h_combo_raw.time_ux).size < 2:
        ...
        hist_20C = filter_Tb(hist, 20.0, batt, tb_band=TB_BAND, rated_batt_cap=rated_batt_cap)
        print("done")
        ...
        if sync_time is None:
            ...
            print("\nresampling ...", end="")
            h_20C_resamp = resample(
                data=hist_20C,
                dt_resamp=dt_resample,
                time_var="time",
                specials=[
                    ("ib_amp_bare_flt", 0),
                    ("ib_noa_bare_flt", 0),
...
def compare_hist_sim(
    ...
    mon_run, sim_run, unit, fault, hist_20C, load_filename, Battery = load_hist_and_prep(
        data_file=data_file,
        time_end=time_end,
        plots=plots,
    ...
    if mon_run is not None and sim_run is not None:
        ...
        replicateOptions = UserOptions(
            ...
            add_voc_mon=dvoc_mon,
            add_voc_sim=dvoc_sim,
            unit=unit,
            use_ib_mon=True,
    ...
    if plots:
        ...
        if hist_20C is not None and len(hist_20C.time) > 1:
            ...
            sim_s_tst = None
            if not S.terse:
                fig_list, fig_files = overall_fault(
                    mon_run,
                    mon_tst,
                    sim_run,
                    sim_tst,
