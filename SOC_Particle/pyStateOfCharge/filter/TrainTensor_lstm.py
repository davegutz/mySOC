...
def plot_result(
    ...
    errors_train = -(predictions_train - actual_train) * scale_in_dv_ * 1000.0
    errors = -(predictions - actual) * scale_in_dv_ * 1000.0
    dv_hys_old_train = np.append(trn_dv_hys_old, val_dv_hys_old)
    dv_hys_old = np.append(dv_hys_old_train, tst_dv_hys_old)
    errors_old_train = -(dv_hys_old_train - actual_train) * scale_in_dv_ * 1000.0
    errors_old = -(dv_hys_old - actual) * scale_in_dv_ * 1000.0
    rows = len(actual)
    # time = range(rows) * t_samp_
    time = []
    for i in range(rows):
    ...
    plt.subplot(2, 1, 1)
    plt.title(
        "Actual and Predicted Values and old model. The Red Line Separates The Training, Validation, and Test Examples"
    )
...
def print_error(trn_y, val_y, tst_y, trn_pred, val_pred, tst_pred, delta_=1.0):
    ...
    # Print
    print(
        "Train    RMSE {:7.5f}".format(trn_rmse),
        " ||  MAE {:7.5f}".format(trn_mae),
        " ||  MRE {:7.5f}".format(trn_mre),
        " || HUB {:7.5f}".format(trn_hub),
    )
    print(
        "Validate RMSE {:7.5f}".format(val_rmse),
        " ||  MAE {:7.5f}".format(val_mae),
        " ||  MRE {:7.5f}".format(val_mre),
        " || HUB {:7.5f}".format(val_hub),
    )
    print(
        "Test     RMSE {:7.5f}".format(tst_rmse),
        " ||  MAE {:7.5f}".format(tst_mae),
        " ||  MRE {:7.5f}".format(tst_mre),
        " || HUB {:7.5f}".format(tst_hub),
    )


def process_battery_attributes(df, scale=(50.0, 10.0, 1.0, 0.1), limit_dv=False):
    ...
    with pd.option_context("mode.chained_assignment", None):
        ...
        y = df["dv"] / s_dv
        if limit_dv:
            y = y.map(lambda x: max(min(x, s_dv * 2.0), -s_dv * 2.0))
    return data, y
...
def tensor_model_create(
    ...
    lstm.add(
        LSTM(
            hidden_units,
            input_shape=simple_input,
            activation=activation[0],
            recurrent_activation=activation[1],
    ...
    if use_two_lstm:
        lstm.add(
            LSTM(hidden_units, activation=activation[0], recurrent_activation=activation[1], return_sequences=True)
        )
        lstm.add(Dropout(drop))
    ...
    final.compile(loss=loss_, optimizer=tf.keras.optimizers.Adam(learning_rate=learn_rate), metrics=["mse", "mae"])
    final.summary()
...
# Single input with one-hot augmentation path
def tensor_model_create_many_to_one(
    hidden_units,
    input_shape,
    activation=None,
    learn_rate=0.01,
    ...
    if use_huber_loss_:
        loss_ = get_huber_loss_fn(delta=huber_delta_)
    lstm.compile(loss=loss_, optimizer=tf.keras.optimizers.Adam(learning_rate=learn_rate), metrics=["mse", "mae"])
    lstm.summary()
...
def train_tensor_lstm():
    ...
    validate_x, validate_y, validate_x_vec = parse_inputs(
        validate_x, validate_y, batch_size, subsample, use_tri_=use_tri
    )
    ...
    if try_load_model:
        ...
        if use_many:
            model = tensor_model_create_many_to_one(
                hidden_units=hidden,
    ...
    plot_input(
        ...
        tst_x=test_x,
        tst_y=test_y,
        t_samp_=t_samp_input,
        use_ib_lag_=use_ib_lag,
        use_tri_=use_tri,
    )
    plot_result(
        trn_y=train_y[:, batch_size - 1, :],
        val_y=validate_y[:, batch_size - 1, :],
        tst_y=test_y[:, batch_size - 1, :],
        trn_pred=train_predict[:, batch_size - 1, :],
        val_pred=validate_predict[:, batch_size - 1, :],
        tst_pred=test_predict[:, batch_size - 1, :],
        trn_dv_hys_old=train_dv_hys_old[:, batch_size - 1, :],
        val_dv_hys_old=validate_dv_hys_old[:, batch_size - 1, :],
        tst_dv_hys_old=test_dv_hys_old[:, batch_size - 1, :],
        t_samp_=t_samp_result,
        scale_in_dv_=scale_in_dv,
    )
...
"""
        es = EarlyStopping(monitor='val_loss', mode='min', verbose=1, patience=50) --> Often, the first sign of no
        further improvement may not be the best time to stop training. This is because the model may coast into a
        plateau of no improvement or even get slightly worse before getting much better. We can account for this by
        adding a delay to the trigger in terms of the number of epochs on which we would like to see no improvement.
        This can be done by setting the “patience” argument.

        es = EarlyStopping(monitor='val_accuracy', mode='max', min_delta=1) --> By default, any change in the
        performance measure, no matter how fractional, will be considered an improvement. You may want to consider an
        improvement that is a specific increment, such as 1 unit for mean squared error or 1% for accuracy. This can be
        specified via the “min_delta” argument.

        es = EarlyStopping(monitor='val_loss', mode='min', baseline=0.4) --> Finally, it may be desirable to only stop
        training if performance stays above or below a given threshold or baseline. For example, if you have familiarity
        with the training of the model (e.g. learning curves) and know that once a validation loss of a given value is
        achieved that there is no point in continuing training. This can be specified by setting the “baseline”
        argument.

        mc = ModelCheckpoint('best_model.h5', monitor='val_loss', mode='min', verbose=1) --> The EarlyStopping callback
        will stop training once triggered, but the model at the end of training may not be the model with best
        performance on the validation dataset. An additional callback is required that will save the best model observed
        during training for later use. This is the ModelCheckpoint callback.
"""
