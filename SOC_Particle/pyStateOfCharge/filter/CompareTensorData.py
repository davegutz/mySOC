...
def adjust_soc(data, dDA_in, scap_in=None):
    ...
    for i in range(n - 1):
        ...
        if scap_in is None:
            ...
            d_soc += dDA_in * T / 360000.0 / scap_in
        data.soc[i + 1] += d_soc
...
def seek_tensor(save_pdf_path="./figures", path_to_temp="./temp"):
    ...
    mon_run, sim_run, f, data_file_clean, temp_flt_file_clean, _ = load_data(
        data_file, skip, unit_key, zero_zero_in, time_end, legacy=legacy_in, v1_only=v1_only_in
    )
    ...
    # How to initialize
    if mon_run.time[0] == 0.0:  # no initialization flat detected at beginning of recording
        if init_time_in:
    ...
    mon_ver, sim_ver, sim_s_ver, mon, sim = replicate(
        mon_run,
        sim_run=sim_run,
        init_time=init_time,
        ...
        slr_res_0=sres0_in,
        stauct_mon=stauct_mon_in,
        slr_res_ct=sresct_in,
    )
    ...
    if plot_overall_in:
        ...
        fig_list, fig_files = off_on_plot(
            mon_run,
            mon_ver,
            sim_run,
            sim_ver,
