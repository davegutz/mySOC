...
class TableInterp1D:
    ...
    def __init__(self, x_, v_):
        ...
        self.x = x_
        if len(v_) != self.n:
            raise Error("length of array 'v' =", len(v_), " inconsistent with length of 'x' = ", len(x_))
        self.v = v_
...
class TableInterp2D:
    ...
    def __init__(self, x_, y_, v_, Dw_=0.0):
        self.n = len(x_)
        self.m = len(y_)
        self.x = x_
        self.y = y_
        ...
        if len(v_) != self.n * self.m:
            raise Error(
                "length of array 'v' =", len(v_), " inconsistent with lengths of 'x' * 'y' = ", len(x_), "*", len(y_)
            )
        self.v = v_
    ...
    def interp(self, x_, y_, printit=False):
        ...
        if printit:
            print(
                f"x {x_} y {y_} high1 {high1} high2 {high2} low1 {low1} low2 {low2} "
                f"temp1 {temp1} temp2 {temp2} dx1 {dx1} dx2 {dx2} r0 {r0} t1 {r1} "
                f"dw {self.Dw} result {result}"
            )
        return result
    ...
    def r_interp(self, t_, y_, verbose=False):
        ...
        if self.is_monotonic:
            ...
            while abs(ice.e) > SOLV_ERR and ice.count < SOLV_MAX_COUNTS and abs(ice.dx) > 0.0:
                ice.increment()
                x_ = ice.x
...
if __name__ == "__main__":  # Example usage: ran ok 20260217
    ...
    t_voc_l = [
        [9.0, 11.8, 12.45, 12.61, 12.8, 12.83, 12.9, 13.00, 13.07, 13.11, 13.23, 13.5],
    ...
    n = 0
    for y in np.arange(t_y_t[0], t_y_t[-1], 0.01):
        for x in np.arange(t_x_soc[0], t_x_soc[-1], 0.01):
            n += 1
            v = table2.interp(x, y)
