...
def plq(
    ...
    if (
        ...
        and hasattr(sx, st)
        and hasattr(sy, yt)
        and getattr(sy, yt) is not None
        and len(getattr(sy, yt)) > 0
        and getattr(sy, yt)[0] is not None
    ):
        try:
        ...
        try:
            if stairs:
                ...
                plt_.stairs(
                    yscld, x_in, color=color, linestyle=linestyle, label=label, baseline=None, linewidth=linewidth
                )
            else:
                plt_.plot(
                    getattr(sx, st),
