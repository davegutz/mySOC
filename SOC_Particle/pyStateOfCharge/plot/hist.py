...
def hs_plots(
    ...
    plq(plt, S.mr, "soc", S.mr, "dv", add=13.0, color="blue", linestyle="-")
    mv.dv = np.array(mv.voc_soc) - np.array(mv.voc)
    plq(plt, S.mv, "soc", S.mv, "dv", add=13.0, color="orange", linestyle="--")
    plt.legend(loc=1)
    plt.subplot(326)
    plq(plt, S.mr, "time", S.mr, "voc_d", color="black", linestyle="-", warn=False)
    plq(plt, S.mr, "time", S.mr, "voc_soc", color="red", linestyle="-")
    plq(plt, S.mv, "time", S.mv, "voc_soc", color="orange", linestyle="--")
    plq(plt, S.mr, "time", S.mr, "dv", add=13.0, color="blue", linestyle="-")
    plq(plt, S.mv, "time", S.mv, "dv", add=13.0, color="orange", linestyle="--")
    plt.legend(loc=1)
    fig_file_name = S.filename + "_" + str(len(fig_list)) + ".png"
    fig_files.append(fig_file_name)
    if S.save_plots and not S.terse:
...
def hs_tune_plots(S: PlotOptions, fig_list=None, fig_files=None, strict_overplot=False):
