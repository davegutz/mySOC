...
def ult_1(S: PlotOptions, fig_files=None, fig_list=None):
    ...
    elif (hasattr(S.mr, "mod") and all(S.mr.mod >= 255.0)) or (
        not (S.strict_overplot) and not (S.run_type == "HistSim" or S.run_type == "HistHist")
    ):
        plq(plt, S.mr, "time", S.mr, "ib_amp_model", add=1.0, color="green", linestyle="-")
        plq(plt, S.mv, "time", S.mv, "ib_amp_model", add=1.0, color="red", linestyle="--", warn=False)
        plq(plt, S.mr, "time", S.mr, "ib_noa_model", add=1.0, color="blue", linestyle="-.")
        plq(plt, S.mv, "time", S.mv, "ib_noa_model", add=1.0, color="orange", linestyle=":", warn=False)
    ...
    plq(
        ...
        color="blue",
        linestyle="--",
        warn=not S.ver_is_stdy and not S.ver_is_sim,
    )
    ...
    # enum  dispw {conn = 0, diff_ib = 1, red_loss = 2, fail_ib = 3, fail_ibm = 4, fail_vb = 5, flt_tb = 6, flt_ekf = 7,
