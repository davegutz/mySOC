...
def sim_s_plots(S: PlotOptions, fig_files=None, fig_list=None):
    ...
    if S.sr and S.smv:
        ...
        plq(plt, S.sv, "time", S.sv, "soc", add=-0.2, color="orange", linestyle="-")
        plq(plt, S.smv, "time", S.smv, "soc_s", add=-0.2, color="orange", linestyle="-")
        plq(plt, S.smv, "time", S.smv, "soc_s", add=-0.2, color="black", linestyle="--")
        plt.legend(loc=1)
        plt.subplot(322)
        if S.mr.vb_hdwe is not None and max(S.mr.vb_hdwe) > 1.0:
