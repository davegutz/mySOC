...
def dom_2(S: PlotOptions, fig_list=None, fig_files=None):
    ...
    plq(
        ...
        linestyle="--",
        stairs=True,
        warn=not S.ver_is_run and not S.ver_is_sim and not S.ver_is_stdy,
    )
...
def ekf_plots(S: PlotOptions, fig_list=None, fig_files=None):
    ...
    if S.sr and S.smv:
        if S.mr.Fx is not None:  # ekf
            ...
            plt.legend(loc=1)
            plt.subplot(312)
            plq(plt, S.mr, "x", S.mr, "hx", color="red", linestyle="-")
            plq(plt, S.mv, "x_ekf", S.mv, "hx", color="black", linestyle="--")
            plt.legend(loc=1)
            plt.subplot(313)
    ...
    if S.mr.voc_soc is not None:
        ...
        plq(
            ...
            markersize="1",
            markevery=64,
            warn=not S.ver_is_run and not S.ver_is_stdy,
        )
        plq(
            plt,
            S.smv,
        ...
        plq(
            plt, S.smv, "time", S.smv, "soc_s", color="cyan", linestyle=":", warn=not S.ver_is_stdy and not S.ver_is_run
        )
        plt.xlabel("sec")
        plt.legend(loc=4)
        if not S.strict_overplot:
            ...
            plq(
                ...
                color="cyan",
                linestyle="--",
                warn=(S.run_type != "HistHist") and (S.run_type != "RunSim") and (S.run_type != "HistSim"),
            )
