...
TB_BAND = 25.0  # Band around temperature to group data and correct
...
def add_stuff_f(
    d_ra, mon, ib_band=0.5, rated_batt_cap=100.0, Dw=0.0, time_sync=None, ap_ib_diff_slr=1.0, ap_ib_quiet_slr=1.0
):
    voc_soc = []
    soc_min = []
    vsat = []
    ...
    for i in range(n):
        ...
        cc_diff_thr_, ewhi_thr_, ewlo_thr_, ib_diff_thr_, ib_quiet_thr_ = fault_thr_bb(
            Tb_f,
            soc,
            voc_soc[i],
            voc_stat_f,
    ...
    time_min = (d_ra.time_ux - time_sync) / 60.0
    time_day = (d_ra.time_ux - time_sync) / 3600.0 / 24.0
...
def fault_thr_bb(Tb, soc, voc_soc, voc_stat, C_rate, bb, ap_ib_diff_slr=1.0, ap_ib_quiet_slr=1.0):
    ...
    cc_diff_sclr_ = 1.0  # ram adjusts during data collection
    ...
    if long_term:
        ...
        class Temp:
            def __init__(self):
                self.str = ""
                self.zero_one = [0, 1]
                self.ptol = [cc_dif_tol, 1.0 + cc_dif_tol]
                self.ntol = [-cc_dif_tol, 1.0 - cc_dif_tol]
        ...
        plt.legend(loc=1)
        plt.subplot(338)
        plq(plt, hi, timestr, hi, "e_wrap_filt", color="black", linestyle="-", marker="o", markersize="3", warn=False)
...
def overall_fault(
    mr, mv, sr, sv, smr, smv, filename, fig_files=None, plot_title=None, fig_list=None, run_type=None, save_plots=False
):
    ...
    plq(
        ...
        color="cyan",
        linestyle="--",
        warn=not run_type == "HistHist" and not run_type == "HistSim",
    )
    plq(
        plt,
        mv,
    ...
    plq(
        ...
        color="black",
        linestyle="-",
        warn=run_type != "HistHist" and not run_type == "HistSim",
    )
    ...
    plq(
        ...
        color="cyan",
        linestyle="--",
        warn=not run_type == "HistSim" and not run_type == "HistSim",
    )
    plq(
        plt,
        smr,
...
def filter_Tb(raw, temp_corr, mon, tb_band=5.0, rated_batt_cap=100.0):
