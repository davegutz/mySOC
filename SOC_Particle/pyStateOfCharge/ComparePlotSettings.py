...
def rescale_time_axes(fig_list, t_min=None, t_max=None):
    ...
    - fig.canvas.draw_idle() triggers a lazy redraw — the window updates on the next GUI event loop cycle, not
    instantly, which is the correct pattern for non-blocking interactive
    use with plt.show(block=False).
    - time_t axes (stair-step temperature subplots) share the same seconds-relative scale as time, so the same
    t_min/t_max applies to both correctly.
