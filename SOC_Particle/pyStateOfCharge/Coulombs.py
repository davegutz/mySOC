...
class Coulombs:
    ...
    def __str__(self, prefix=""):
        ...
        s += (
            "  soc =        {:7.3f}       // Fraction of saturation charge (q_capacity_) available (0-1)  soc)\n"
        ).format(self.soc)
        s += (
            "  sat =          {:1.0f}          // Indication from caller that battery is saturated, T=saturated\n"
        ).format(self.sat)
        ...
        s += (
            "  tweak_test =    {:d}          // Driving signal injection completely using softwareinj_soft_bias\n"
        ).format(self.tweak_test)
        s += (
            "  coul_eff =   {:8.4f}      // Coulombic efficiency- fraction of charging turned into usableCoulombs\n"
    ...
    def calculate_capacity(self, tb_f):
        """Capacity"""
        try:
            res = self.q_cap_rated_scaled * (1.0 + self.chemistry.dqdt * (tb_f - self.chemistry.rated_temp))
            if self.reset:
                pass
            else:
                pass
    ...
    def count_coulombs(self, OPT, chem, dt, reset, tb_f, charge_curr, sat, saturated):
        ...
        if charge_curr > 0.0 and not self.tweak_test:
        ...
        # if charge_curr < 0.:
        #     print(f"{OPT.mon_run.time[G.i]=} {OPT.mon_run.time[G.i]-OPT.mon_run.time[G.i-1]=} {OPT.mon_run.dt[G.i]=}
        # {dt} {OPT.mon_run.ib_charge[G.i]=} {charge_curr} {OPT.mon_run.ib_charge[G.i] * OPT.mon_run.dt[G.i]=} {dt *
        # charge_curr} {OPT.mon_run.d_delta_q[G.i]=} {self.d_delta_q}")
        ...
        # Save and return
        # print('Mon CC:  charge_curr', charge_curr, 'dt', dt, 'd_delta_q', d_delta_q,'temp_lim', self.temp_lim, 'cap',
        # self.q_capacity)
        return self.soc
