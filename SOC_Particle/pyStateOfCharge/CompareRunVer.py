...
def compare_pair(run_path, ver_path, tol, rtol=1e-3, ed=None):
    ...
    if "reset" in df_ver.columns:
        reset_mask = df_ver["reset"].astype(str).str.lower().isin(["true", "1", "1.0"]) & (df_ver["time"] > 1.0)
    ...
    numeric_cols = [
        c
        for c in shared_cols
        if pd.api.types.is_numeric_dtype(df_run[c])
        and pd.api.types.is_numeric_dtype(df_ver[c])
        and not pd.api.types.is_bool_dtype(df_run[c])
        and not pd.api.types.is_bool_dtype(df_ver[c])
    ]
...
def report(results, tol, rtol=1e-3, option="", macro=""):
    ...
    print(f"\n{'=' * 72}")
    print(
        f"  CompareRunVer  |  tol={tol}  rtol={rtol}  |  option={option}  |  macro={macro}  |  {len(results)} pair(s)"
    )
    print(f"{'=' * 72}\n")
    ...
    for r in results:
        ...
        for d in r["diffs"]:
            print(
                f"    {d['param']:<30}  {d['n_bad']:>6}  "
                f"{d['max_diff']:>12.6f}  {d['mean_diff']:>12.6f}  "
                f"{d['first_time']:>10.3f}"
            )
        print()
...
def plot_diffs(results, data_file=None, save_plots=True, terse=False, hardcopy=True, show_killer_=True):
    ...
    show_killer(
        ...
        pdf_base=os.path.join(save_pdf_path, "CompareRunVer"),
        hardcopy=hardcopy,
    )
