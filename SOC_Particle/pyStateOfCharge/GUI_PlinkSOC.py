...
# The #! operates for macOS only. 'Python Launcher' (Python Script Preferences) option for 'Allow override with #! in
# script' is checked.
...
#
# noinspection
# PyTypeChecker, PyArgumentList, PyCallingNonCallable, PyUnfilledParameters, SpellCheckingInspection, PyPep8Naming,
# PyUnboundLocalVariable, PyShadowingNames, PyShadowingBuiltins
# type: ignore
...
class Exec:
    def __init__(self, cf_=None, ind=None, level=None, path_disp_len_=25):
        ...
        self.folder_button = myButton(
            master,
            text=self.dataReduction_folder[-20:],
            command=self.enter_data_reduction_folder,
            fg="blue",
            bg=bg_color,
        )
    ...
    def enter_battery(self):
        answer = tk.simpledialog.askstring(
            title=self.level, prompt="Enter battery e.g. 'bb for Battleborn', 'ch' or 'chg' for CHINS:"
        )
    ...
    def enter_data_reduction_folder(self):
        answer = tk.filedialog.askdirectory(
            title="Select a destination (i.e. Library) dataReduction folder", initialdir=self.dataReduction_folder
        )
    ...
    def enter_version(self):
        answer = tk.simpledialog.askstring(
            title=__file__, prompt="Enter version <vYYYYMMDD>:", initialvalue=self.version
        )
        if answer is None or answer == () or answer == "":
            print("enter operation cancelled")
            return
...
def compare_hist_hist_choose():
    ...
    testpaths = filedialog.askopenfilenames(
        title="Choose test file(s)", filetypes=[("csv", ".csv")], initialdir=Test.dataReduction_folder
    )
    if testpaths is None or testpaths == "":
        ...
        for testpath in testpaths:
            ...
            if test_key != "":
                run_path = filedialog.askopenfilename(
                ...
                compare_hist_hist(
                    data_file_run=run_path,
                    unit_key_run=ref_key,
                    data_file_tst=testpath,
                    unit_key_tst=test_key,
                    dt_resample=30.0,
                    terse=terse.get(),
                )
            else:
...
def compare_hist_sim_choose():
    ...
    testpaths = filedialog.askopenfilenames(
        title="Please select files", filetypes=[("csv", ".csv")], initialdir=Test.dataReduction_folder
    )
    if testpaths is None or testpaths == "":
        print("No file chosen")
    else:
        ...
        for testpath in testpaths:
            ...
            if key != "":
                answer = tk.simpledialog.askinteger(
                    title=__file__, prompt="Simulation re-construction sample time in seconds", initialvalue=900
                )
                if answer is None:
                    print("enter operation cancelled")
                    return
...
def compare_hist_to_sim():
    ...
    if modeling.get():
        ...
        answer = tk.simpledialog.askinteger(
            title=__file__, prompt="Simulation re-construction sample time in seconds", initialvalue=10
        )
        if answer is None:
            print("enter operation cancelled")
            return
...
def compare_run(show_killer_=True):
    ...
    if modeling.get():
        ...
        return compare_run_run(
            keys=keys,
            data_file_folder_run=Ref.version_path,
            data_file_folder_test=Test.version_path,
            terse=terse.get(),
            hardcopy=hardcopy.get(),
        )
...
def compare_run_to_hist():
    ...
    if modeling.get():
        print("compare_hist_to_sim.  save_pdf_path", str(PurePosixPath(Test.version_path) / "figures"))
        compare_run_hist(
            data_file=Test.file_path, unit_key=Test.key, strict_overplot=strict_overplot.get(), terse=terse.get()
        )
    else:
        print("not possible")
...
def compare_hist_hist_run():
    ...
    compare_hist_hist(
        ...
        data_file_tst=Test.file_path,
        unit_key_tst=Test.key,
        dt_resample=answer,
        terse=terse.get(),
        hardcopy=hardcopy.get(),
    )
...
def enter_mod_in_app():
    answer = tk.simpledialog.askinteger(
        title=__file__, prompt="enter the value of Modeling in app to assume", initialvalue=mod_in_app.get()
    )
    if answer is None:
        print("enter operation cancelled")
        return
...
def grab_init(command_to_append="", force_if_ready=False, force_kill=False, fg_color="#ffffff"):
    ...
    return start_plink(
        command_to_paste=init_command, force_if_ready=force_if_ready, force_kill=force_kill, fg_color=fg_color
    )
...
def grab_start():
    ...
    if look_plink(platform.system()):
        if not is_plink_ready():
            ...
            tkinter.messagebox.showinfo(
                title="Not Ready", message="Please wait until terminal is READY or run the START HERE button."
            )
            return
    ...
    grab_all_nominal()
    save_data_button.config(
        bg=bg_color, activebackground=bg_color, fg="black", activeforeground="black", text="save data"
    )
    save_data_as_button.config(
        bg=bg_color, activebackground=bg_color, fg="black", activeforeground="black", text="save data as"
    )
    grab_all_nominal()
...
def grab_time():
    ...
    grab_all_nominal()
    get_time_button.config(
        bg="yellow", activebackground="yellow", fg="black", activeforeground="black", text=current_ut
    )
...
def kill_plink(sys_=None, silent=True):
    ...
    if plink_pid:
        if sys_ == "Windows":
            ...
            if comm != "plink":
                print(
                    f"plink_pid {plink_pid} now names {comm!r}, not 'plink' — "
                    "skipping targeted kill, falling back to pkill."
                )
                plink_pid = None
            else:
...
def grab_auto():
    ...
    if errors:
        tkinter.messagebox.showerror(title="AUTO Prerequisites Not Met", message="\n\n".join(errors))
        return
...
def save_data(show_killer_=True):
    ...
    if size_of(plink_test_csv_path.get()) > 64:  # bytes
        ...
        if Path(Test.file_path).is_file() and Path(Test.file_path).stat().st_size > 0:  # bytes
            if auto_overwrite.get() or _was_auto:
                ...
                if not confirmation:
                    ...
                    return
        save_data_button.config(
            bg="yellow", activebackground="yellow", fg="black", activeforeground="black", text="data saving"
        )
        tksleep(0.1)
        if run_start_time is not None:
            elapsed = time.time() - run_start_time
        ...
        if timer is not None:
            ...
            timer = None
        save_data_button.config(
            bg="green", activebackground="green", fg="red", activeforeground="red", text="data saved"
        )
...
def save_data_as():
    ...
    if size_of(plink_test_csv_path.get()) > 512:  # bytes
        ...
        if option.get() == "custom":
            ...
            new_file_txt = tk.simpledialog.askstring(
                title=__file__, prompt="custom file name string:", initialvalue=Test.file_txt
            )
            if new_file_txt is not None:
                Test.create_file_path_and_key(name_override=new_file_txt)
                Test.label.config(text=Test.file_txt)
        ...
        if timer is not None:
            ...
            timer = None
        save_data_as_button.config(
            bg="green", activebackground="green", fg="red", activeforeground="red", text="data saved as"
        )
        empty_file(plink_test_csv_path.get())
        print("updating Test file label")
        Test.create_file_path_and_key(name_override=new_file_txt)
...
def save_plink():
    m_str = (
        datetime.datetime.fromtimestamp(Path(plink_test_csv_path.get()).stat().st_mtime)
    ...
    if enter_size < 64:
        ...
        if platform.system() == "Linux":
            ...
            if "gnome-terminal" in term:
                ...
                cmd = [
                    term,
                    "--app-id=local.plink-terminal-server",
                    "--zoom=0.8",
                    "--",
                    "bash",
                    "-c",
            ...
            bat_path = os.path.join(
                os.environ.get("TEMP", os.environ.get("TMP", "C:\\Temp")), f"plink_run_{int(time.time() * 1000)}.bat"
            )
            ...
            proc = subprocess.Popen(["cmd", "/k", bat_path], creationflags=subprocess.CREATE_NEW_CONSOLE)
            ...
            try:
                ...
                out = subprocess.check_output(
                    ["tasklist", "/FI", "IMAGENAME eq plink.exe", "/NH", "/FO", "CSV"]
                ).decode("ascii")
            ...
            if command_to_paste:
                plink_cmd = (
                    f"(echo '{command_to_paste}'; cat) | plink -batch -T -load {test_filename.get()} | "
                    f"tee {plink_test_csv_path.get()}"
                )
            else:
...
if __name__ == "__main__":  # Example usage.  Ran ok 20260217
    ...
    modeling_button = tk.Checkbutton(
        top_panel_right_ctr, text="modeling", bg=bg_color, variable=modeling, onvalue=True, offvalue=False
    )
    ...
    Test.folder_button = myButton(
        top_panel_left_ctr,
        text=Test.dataReduction_folder[-folder_reveal:],
    ...
    auto_overwrite_button = tk.Checkbutton(
        top_panel_right_ctr, text="auto over-write", bg=bg_color, variable=auto_overwrite, onvalue=True, offvalue=False
    )
    auto_overwrite_button.pack(pady=2, fill="x")
    auto_overwrite.trace_add("write", handle_auto_overwrite)

    Ref.folder_button = myButton(
        top_panel_right,
        text=Ref.dataReduction_folder[-folder_reveal:],
    ...
    Test.version_button = myButton(
        top_panel_left_ctr, text=Test.version, command=Test.enter_version, fg="blue", bg=bg_color
    )
    ...
    open_plink_button = myButton(
        init_row_frame,
        text="Open\nPlink",
        command=open_plink_window,
        fg="#F5DEB3",
        bg="#2F4F4F",
        activeforeground="wheat",
        activebackground="#8BA88B",
        font=("Arial", 8),
    )
    ...
    if platform.system() == "Darwin":
        start_button = myButton(
            option_panel_ctr, text="", command=grab_start, fg="purple", bg=bg_color, justify="left", font=butt_font
        )
        run_sim_all_button = myButton(
            auto_buttons_row,
            text="RunSimAll",
            command=run_sim_all_batch,
            fg="blue",
            bg=bg_color,
            justify="left",
            font=butt_font,
    ...
    auto_button = myButton(
        auto_buttons_row, text="AUTO", command=grab_auto, fg="blue", bg=bg_color, justify="left", font=butt_font
    )
    ...
    auto_plink_entry = tk.Entry(
        auto_group,
        textvariable=auto_plink_path_var,
        state="readonly",
        font=label_font_gentle,
    ...
    documentation_button = tk.Checkbutton(
        option_panel_right, text="Documentation", variable=documentation, onvalue=True, offvalue=False
    )
    ...
    get_time_button = myButton(
        macro_panel_right, text="grab time copy/paste buffer", command=grab_time, fg="blue", bg=bg_color
    )
    ...
    strict_overplot_button = tk.Checkbutton(
        sav_panel, text="strict_overplot plots", variable=strict_overplot, onvalue=True, offvalue=False
    )
    ...
    clear_data_button = myButton(
        sav_panel,
        text="clear",
        command=clear_data_verbose,
        fg="red",
        bg=bg_color,
        wraplength=wrap_length,
        justify="right",
    ...
    save_data_as_button = myButton(
        sav_panel, text="save as", command=save_data_as, fg="red", bg=bg_color, wraplength=wrap_length, justify="left"
    )
    ...
    if platform.system() == "Darwin":
        ...
        hist_hist_button = myButton(
            run_panel,
            text="Compare Hist Hist",
            command=compare_hist_hist_run,
            fg="green",
            bg=bg_color,
            justify="left",
            font=butt_font_large,
        )
        hist_sim_button = myButton(
            run_panel,
            text=" Compare ",
            command=compare_hist_to_sim,
            fg="green",
            bg=bg_color,
            justify="left",
            font=butt_font_large,
        )
        run_sim_hist_button = myButton(
            run_panel,
            text=" Compare ",
            command=compare_run_to_hist,
            fg="green",
            bg=bg_color,
            justify="left",
            font=butt_font_large,
    ...
    run_sim_choose_button = myButton(
        compare_panel,
        text="Compare Run Sim Choose",
        command=compare_run_sim_choose,
        fg="blue",
        bg=bg_color,
        wraplength=wrap_length,
        justify="left",
    ...
    run_run_choose_button = myButton(
        compare_panel,
        text="Compare Run Run Choose",
        command=compare_run_run_choose,
        fg="blue",
        bg=bg_color,
        wraplength=wrap_length,
        justify="left",
    ...
    run_sim_choose_button = myButton(
        compare_panel,
        text="Compare Hist Sim Choose",
        command=compare_hist_sim_choose,
        fg="blue",
        bg=bg_color,
        wraplength=wrap_length,
        justify="left",
    ...
    run_sim_choose_button.pack(side="left", padx=5, pady=5)
    hist_hist_choose_button = myButton(
        compare_panel,
        text="Compare Hist Hist Choose",
        command=compare_hist_hist_choose,
        fg="blue",
        bg=bg_color,
        wraplength=wrap_length,
        justify="left",
