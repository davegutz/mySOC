...
A few notes on the function:
  - line1.set_ydata(y1_data) can also be switched to line1.set_data(x_vec,y1_data) to change both x and y data on the
  plots.
  - plt.pause() is necessary to allow the plotter to catch up - I've been able to use a pause time of 0.01s without any
  issues
...
def liven_plotter(x, y, linen, fig, labels=None, subplot=111, ax=None, y_label="", title="", pause_time=0.1, symbol=""):
    if linen is None:
...
if __name__ == "__main__":  # Example usage.  Ran ok 20260217
    ...
    def main():
        ...
        y_vec2 = np.random.randn(len(x_vec), 1) * 10.0
        linen_x1 = None
        linen_x2 = None
        axx1 = None
        axx2 = None
        ...
        while True and count < count_max:
            ...
            y_vec2[-1][0] = np.random.randn(1) * 10.0
            ...
            linen_x2, axx2 = liven_plotter(
                x_vec, y_vec2, linen_x2, fign, subplot=212, ax=axx2, y_label="Y Label2", pause_time=0.1, labels=["v21"]
            )
