...
lookup = {
        ...
        'rapidTweakRegressionH0': (230, 'Sh0;' + slow + 'Rs;W4;Xp10;Pf;W2;' + quiet + cleanup + 'Sh1;<XD;', ('Should run three very large current discharge/recharge cycles without fault', 'No hysteresis. Best test for seeing time skews and checking fault logic for false trips', 'Tease out cause of e_wrap faults.  e_wrap MUST be flat!', 'Occasional jumps in ib_sel_stat are normal when pass through 0 A')),
        'offLowSoc': (160, modEmptInitGen + tranPrep  + vm12 + 'XQ55000;' + dv0 + quiet + cleanup + '<XD;', ('Test for clean faults on shutoff.',)),
        'offSitBmsBB': (670, modEmptInitBB + slowTwitchDef + 'Xa-162;' + tranPrep + twitch + 'XQ568000;' + 'Pf;W2;Xa0;' + quiet + cleanup + 'Sh1;<XD;', ('for CompareRunRun.py Argon vs Photon builds. This is the only test for that.',)),
        'offSitBmsCHG': (670, modEmptInitCHG + slowTwitchDef + 'Xa-324;' + tranPrep + twitch + 'XQ568000;' + 'Pf;W2;Xa0;' + quiet + cleanup + 'Sh1;<XD;', ('for CompareRunRun.py Argon vs Photon builds. This is the only test for that.',)),
