...
def replace(filepaths=None, silent=False, supported="*"):
    ...
    default_dict = {
        "replace": {"source": "source", "target": "target", "filepaths": ["file1", "file2", "file3"]},
    }
    ...
    source = tk.simpledialog.askstring(
        "replace source target <files>", "source string", initialvalue=cf.get_item("replace", "source")
    )
    cf.put_item("replace", "source", source)
    target = tk.simpledialog.askstring(
        "replace source target <files>", "target string", initialvalue=cf.get_item("replace", "target")
    )
