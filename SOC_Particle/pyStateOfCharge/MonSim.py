...
def replicate(OPT: UserOptions):
    ...
    if hasattr(OPT.mon_run, "time_run_start"):
        ...
        mon.saved.time_run_start = 0.0
        sim.saved_s.time_run_start = 0.0
    ...
    while G.i < t_len - 1:
        ...
        if hasattr(OPT.mon_run, "time_t"):
            ...
            if (
                hasattr(OPT.mon_run, "mtb")
                and OPT.mon_run.mtb is not None
                and OPT.mon_run.mtb[G.i] > 0.0
                and G.i + 1 < n
            ):
        ...
        if consider_ekf:
            i_ekf += 1
            reset_ekf = i_ekf < 1 or reset or OPT.run_type == "HistSim"
            if i_ekf < 1:
        ...
        mon.count_coulombs(
            OPT, chem=_chm_m, dt=T, reset=reset, tb_f=Tb_f[G.i], charge_curr=ib_charge, sat=sat, saturated=saturated
        )
        ...
        if hasattr(OPT.mon_run, "Tb_fa") and bool(OPT.mon_run.Tb_fa[G.i]) and mon.q_capacity and mon.q_capacity != 0.0:
        ...
        if SN.run_type == "RunSim":
            if (
                SN.mon_run.skip_ekf[i_ekf]
                or SN.mon_run.skip_sel[G.i]
                or SN.mon_run.skip_mon[G.i]
    ...
    if OPT.verbose:
        print(
            "   time mr.chm sr.chm sr.ib_in_s sr.dv_hys  mr.ib mr.soc mr.dv_hys   smv.ib_in_s sim.ibs sim.ioc"
            "sim.sat sim.dis sim.dv_dot smv.dv_hys  mv.ib  mv.soc mon.ibs  mon.ioc   mon.sat   mon.dis   "
            "mon.dv_dot  mv.dv_hys"
        )
        print("time=", now)
        print("mon:  ", str(mon))
        print("sim:  ", str(sim))
