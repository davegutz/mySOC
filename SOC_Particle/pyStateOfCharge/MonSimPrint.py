...
def print_pair(val1, val2, total_digits, sig_digits, name, print_name, df=False, end="", color=None, tol=1e-3, rtol=1e-3):
    ...
    if val2 is None:
        total_len = max(total_digits, len(name) + 1)
    else:
        total_len = max(2 * total_digits, len(name) + 1)
    ...
    if print_name:
        ...
        if val2 is None:
            out_str = _fmt(val1, total_len)
        else:
            s1 = _fmt(val1, total_digits)
            digits2 = total_len - total_digits
            s2 = _fmt(val2, digits2)
            out_str = f"{s1}{s2}"
            if df and val1 is not None and val2 is not None:
                if not (isinstance(val1, str) or isinstance(val2, str)):
                    is_different = False
                    try:
...
def print_col_leads(h, df, t, SN, mon, sim, i_temp, calc_temp, i_ekf, calc_ekf):
    print_pair(G.i, None, 8, 0, 'i', h, df)
    print_pair(t[G.i], None, 13, 3, 'time', h, df)
    ...
    print_pair(bool(cp_ekf_reset_val[i_ekf]) if cp_ekf_reset_val is not None else False, mon.reset_ekf, 2, 0, 're', h,
               df)
    print_pair(i_temp, None, 6, 0, 'it', h, df)
    print_pair(calc_temp, None, 2, 0, 'ct', h, df)
    print_pair(i_ekf, None, 6, 0, 'ie', h, df)
...
def print_cutback_RunSim(SN, i_temp, i_ekf, t, mon, sim, calc_temp, calc_ekf, df=False):
    ...
    for i_hdr in range(int(print_hdr), -1, -1):
        ...
        print_pair(SN.mon_run.ib_amp_hdwe[G.i], mon.ib_amp_hdwe, 13, 8, 'ib_amp_hdwe', h, df)
        print_pair(SN.mon_run.ib_noa_hdwe[G.i], mon.ib_noa_hdwe, 13, 8, 'ib_noa_hdwe', h, df)
        print_pair(SN.mon_run.ib_amp_model[G.i], mon.ib_amp_model, 13, 8, 'ib_amp_model', h, df)
        print_pair(SN.mon_run.ib_noa_model[G.i], mon.ib_noa_model, 13, 8, 'ib_noa_model', h, df)
        print_pair(SN.sim_run.ib_in_s[G.i], sim.ib_in, 13, 8, 'ib_in', h, df)
        print_pair(SN.sim_run.ib_s[G.i], sim.ib, 13, 8, 'ib_s', h, df)
        print_pair(SN.sim_run.soc_s[G.i], sim.soc, 13, 8, 'soc_s', h, df)
        print_pair(SN.sim_run.sat_ib_max_s[G.i], sim.sat_ib_max, 13, 8, 'sat_ib_max', h, df)
        print_pair(SN.sim_run.voc_stat_s[G.i], sim.voc_stat, 13, 8, 'voc_stat_s', h, df)
        print_pair(SN.sim_run.vsat_s[G.i], sim.vsat, 13, 8, 'vsat_s', h, df)
        print_pair(SN.sim_run.ib_charge_s[G.i], sim.ib_charge, 13, 8, 'ib_charge_s', h, df)
        print_pair(SN.sim_run.dv_dyn_s[G.i], sim.dv_dyn, 13, 8, 'dv_dyn_s', h, df)
        print_pair(SN.mon_run.ib[G.i], mon.ib, 13, 8, 'ib', h, df)
        print_pair(SN.mon_run.ib_charge[G.i], mon.ib_charge, 13, 8, 'ib_charge', h, df)
        print_pair(SN.mon_run.d_delta_q[G.i], mon.d_delta_q, 13, 8, 'd_delta_q', h, df)
        print_pair(SN.mon_run.delta_q[G.i], mon.delta_q, 13, 8, 'delta_q', h, df, end="\n")
...
def print_dyn_m_RunSim(SN, i_temp, i_ekf, t, mon, sim, calc_temp, calc_ekf, df=False):
    ...
    for i_hdr in range(int(print_hdr), -1, -1):
        ...
        print_pair(bool(SN.mon_run.ib_amp_lo[G.i]), bool(mon.ib_amp_lo), 2, 0, 'ib_amp_lo', h, df)
        print_pair(bool(SN.mon_run.ib_amp_hi[G.i]), bool(mon.ib_amp_hi), 2, 0, 'ib_amp_hi', h, df)
        print_pair(bool(SN.mon_run.ib_lo_active[G.i]), mon.ib_lo_active, 2, 0, 'ib_lo_active', h, df)
        print_pair(bool(SN.mon_run.disable_amp_fault[G.i]), bool(mon.disable_amp_fault), 2, 0, 'dis_amp_flt', h, df)
        ...
        print_pair(bool(SN.mon_run.ib_wrp_reset_m[G.i]), bool(mon.LoopIbAmp.WrapErrFilt.reset), 2, 0, 'e_wrap_m_reset', h, df)
        ...
        print_pair(SN.mon_run.wrap_hi_m_flt[G.i], mon.wrap_hi_m_flt, 2, 0, 'e_wrap_m_flt', h, df)
        print_pair(SN.mon_run.wrap_hi_m_fa[G.i], mon.wrap_hi_m_fa, 2, 0, 'e_wrap_m_fa', h, df)
        print_pair(SN.mon_run.disable_amp_fault[G.i], mon.disable_amp_fault, 2, 0, 'disable_amp_fault', h, df)
        print_pair(mon.ib_amp_lo, None, 2, 0, 'ib_amp_lo', h, df)
        print_pair(SN.mon_run.e_wrap_m_reset[G.i], mon.e_wrap_m_reset, 2, 0, 'e_wrap_m_reset', h, df)
...
def print_dyn_n_RunSim(SN, i_temp, i_ekf, t, mon, sim, calc_temp, calc_ekf, df=False):
    ...
    for i_hdr in range(int(print_hdr), -1, -1):
        ...
        print_pair(bool(SN.mon_run.ib_noa_lo[G.i]), bool(mon.ib_noa_lo), 2, 0, 'ib_noa_lo', h, df)
        print_pair(bool(SN.mon_run.ib_noa_hi[G.i]), bool(mon.ib_noa_hi), 2, 0, 'ib_noa_hi', h, df)
        print_pair(SN.mon_run.dt[G.i], mon.dt, 11, 4, 'dt', h, df)
        ...
        print_pair(bool(SN.mon_run.ib_wrp_reset_n[G.i]), bool(mon.LoopIbNoa.WrapErrFilt.reset), 2, 0, 'e_wrap_n_reset', h, df)
        ...
        print_pair(SN.mon_run.wrap_hi_n_flt[G.i], mon.wrap_hi_n_flt, 2, 0, 'e_wrap_n_flt', h, df)
        print_pair(SN.mon_run.wrap_hi_n_fa[G.i], mon.wrap_hi_n_fa, 2, 0, 'e_wrap_n_fa', h, df)
        print_pair(bool(SN.mon_run.ib_noa_lo[G.i]), None, 2, 0, 'ib_noa_lo', h, df)
        print_pair(SN.mon_run.fltw[G.i], None, 18, 0, 'fltw', h, df)
...
def print_ekf_RunSim(SN, i_temp, i_ekf, t, mon, sim, calc_ekf, calc_temp, df=False):
    ...
    for i_hdr in range(int(print_hdr), -1, -1):
        ...
        print_pair(bool(SN.mon_run.voltage_low[G.i]), bool(mon.voltage_low), 2, 0, 'voltage_low', h, df)
        print_pair(bool(SN.mon_run.bms_off[G.i]), bool(mon.bms_off), 2, 0, 'bms_off', h, df)
        print_pair(SN.mon_run.frz[i_ekf], mon.frz, 2, 0, 'frz', h, df)
        ...
        print_pair(mon.x, None, 13, 8, 'mon.x', h, df)
        ...
        print_pair(SN.mon_run.Tb_f[G.i], mon.Tb_f, 13, 8, 'Tb_f', h, df)
        print_pair(SN.mon_run.x_prior[i_ekf], mon.x_prior, 13, 8, 'x_prior', h, df)
...
def print_soc_s_HistSim(SN, i_temp, t, mon, sim, calc_temp, i_ekf, calc_ekf, df=False):
    ...
    for i_hdr in range(int(print_hdr), -1, -1):
        ...
        print_pair(SN.mon_run.vsat[G.i], mon.vsat, 11, 5, 'vsat', h, df)
        print_pair(bool(SN.sim_run.bms_off_s[G.i]), sim.bms_off, 2, 0, 'bms_off_s', h, df, end="\n")
...
def print_soc_s_RunSim(SN, i_temp, t, mon, sim, calc_temp, i_ekf, calc_ekf, df=False):
    ...
    for i_hdr in range(int(print_hdr), -1, -1):
        ...
        print_pair(SN.sim_run.dv_dyn_s[G.i], sim.dv_dyn, 11, 5, 'dv_dyn_s', h, df)
        print_pair(SN.mon_run.vsat[G.i], mon.vsat, 11, 5, 'vsat', h, df)
        print_pair(bool(SN.sim_run.bms_off_s[G.i]), sim.bms_off, 2, 0, 'bms_off_s', h, df)
        print_pair(bool(SN.sim_run.voltage_low_s[G.i]), sim.voltage_low, 2, 0, 'voltage_low_s', h, df, end="\n")
...
def print_volt_RunSim(SN, i_temp, i_ekf, t, mon, sim, calc_temp, calc_ekf, df=False):
    ...
    for i_hdr in range(int(print_hdr), -1, -1):
        ...
        print_pair(SN.mon_run.dt[G.i], mon.dt, 9, 4, 'dt', h, df)
        print_pair(SN.mon_run.vb[G.i], mon.vb, 13, 7, 'vb', h, df)
        print_pair(SN.mon_run.Tb_f[G.i], mon.Tb_f, 14, 8, 'Tb_f', h, df)
        ...
        print_pair(bool(SN.mon_run.disable_amp_fault[G.i]), bool(mon.Diff.disable_amp_fault), 2, 0, 'disable_amp_fault', h, df)
        print_pair(SN.mon_run.ib_diff[G.i], mon.Diff.ib_diff, 14, 6, 'ib_diff', h, df)
        print_pair(SN.mon_run.ib_diff_f[G.i], mon.Diff.ib_diff_f, 14, 6, 'ib_diff_f', h, df)
        print_pair(SN.mon_run.ib_diff_T[G.i] if hasattr(SN.mon_run, 'ib_diff_T') else None, mon.Diff.ib_diff_T, 14, 6, 'ib_diff_T', h, df)
        print_pair(SN.mon_run.ib_diff_rstate[G.i] if hasattr(SN.mon_run, 'ib_diff_rstate') else None, mon.Diff.ib_diff_rstate, 14, 6, 'ib_diff_rstate', h, df)
        print_pair(SN.mon_run.ib_diff_state[G.i] if hasattr(SN.mon_run, 'ib_diff_state') else None, mon.Diff.ib_diff_state, 14, 6, 'ib_diff_state', h, df)
        print_pair(SN.mon_run.ib_diff_tau[G.i] if hasattr(SN.mon_run, 'ib_diff_tau') else None, mon.Diff.ib_diff_tau, 14, 6, 'ib_diff_tau', h, df)
        print_pair(SN.mon_run.ib_diff_flt[G.i], mon.Diff.ib_diff_hi_flt or mon.Diff.ib_diff_lo_flt, 2, 0, 'ib_diff_flt', h, df)
        print_pair(bool(SN.mon_run.ib_lo_active[G.i]), mon.ib_lo_active, 2, 0, 'ib_lo_active', h, df)
        print_pair("x", mon.ib_lo_limited_hi, 2, 0, 'ib_lo_limited_hi', h, df)
        print_pair("x", mon.ib_lo_limited_lo, 2, 0, 'ib_lo_limited_lo', h, df)
        ...
        print_pair(SN.mon_run.e_wrap_filt[G.i], mon.e_wrap_filt, 11, 5, 'e_wrap_filt', h, df)
        print_pair(bool(SN.mon_run.ib_dyn_r[G.i]), mon.ib_dyn_r, 2, 0, 'ib_dyn_r', h, df)
