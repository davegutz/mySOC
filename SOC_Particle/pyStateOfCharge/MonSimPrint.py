...
def print_volt_RunSim(SN, i_temp, i_ekf, t, mon, sim, calc_temp, calc_ekf, df=False):
    ...
    for i_hdr in range(int(print_hdr), -1, -1):
        ...
        print_pair(bool(SN.mon_run.disable_amp_fault[G.i]), bool(mon.disable_amp_fault), 2, 0, 'disable_amp_fault', h, df)
        print_pair(SN.mon_run.ib_diff[G.i], mon.ib_diff, 14, 6, 'ib_diff', h, df)
        print_pair(SN.mon_run.ib_diff_flt[G.i], mon.ib_diff_hi_flt or mon.ib_diff_lo_flt, 2, 0, 'ib_diff_flt', h, df)
        print_pair(bool(SN.mon_run.ib_lo_active[G.i]), mon.ib_lo_active, 2, 0, 'ib_lo_active', h, df)
...
def print_vb_wrap_RunSim(SN, i_temp, i_ekf, t, mon, sim, calc_temp, calc_ekf, df=False):
    ...
    for i_hdr in range(int(print_hdr), -1, -1):
        ...
        print_pair(SN.mon_run.e_wrap_m_filt[G.i], mon.e_wrap_m_filt, 11, 5, 'e_wrap_m_filt', h, df)
        print_pair(SN.mon_run.ib_diff_fa[G.i], mon.ib_diff_hi_fa or mon.ib_diff_lo_fa, 2, 0, 'ib_diff_fa', h, df)
