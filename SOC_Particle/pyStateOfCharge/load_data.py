...
def remove_0T(d_ra, info):
    ...
    condition = d_ra["time_ux"] >= 1746684850783.0 / 1000.0
...
def load_data(
    ...
    zero_zero,
    time_end,
    rated_batt_cap=Battery.NOM_UNIT_CAP,
    legacy=False,
    zero_thr_in=0.02,
    init_time=None,
    time_shift=None,
    ...
    battery_file_clean = write_clean_file(
        path_to_data, type_="_battery", hdr_key=battery_hdr, unit_key=battery_val, skip=skip
    )
    ...
    temp_flt_file_clean = write_clean_file(
        path_to_data, type_="_flt", hdr_key="fltb", unit_key="unit_f", skip=skip, comment_str="---"
    )
    ...
    # Load ekf (ref)
    ekf_file_clean = write_clean_file(path_to_data, type_="_ekf", hdr_key=hdr_key, unit_key=unit_key_ekf, skip=skip)
    if ekf_file_clean:
    ...
    # Load shunt (ref)
    shunt_file_clean = write_clean_file(
        path_to_data, type_="_shunt", hdr_key=hdr_key_shunt, unit_key=unit_key_shunt, skip=skip
    )
    if shunt_file_clean:
    ...
    data_file_sim_clean = write_clean_file(
        path_to_data, type_="_sim", hdr_key=hdr_key_sim, unit_key=unit_key_sim, skip=skip
    )
    if data_file_sim_clean:
        ...
        sim_raw = None
        sim = SavedDataSim(
            time_run_start=mon.time_run_start,
            data=sim_raw,
            time_end=time_end,
            fake=True,
            mon_for_fake=mon,
