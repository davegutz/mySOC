...
class Wave:
    ...
    def __init__(
        self,
        datav=np.array([0.0, 0.0]),
        timev=np.array([0.0, 1.0]),
        sample_hz=1.0,
        ss_time_rng=np.array([0.0, 1.0]),
        tool_lag=None,
        data_lag_nyquist_ratio=50.0,
        sigma_factor=6.0,
        sweep_s=None,
    ):
        """
        ...
        self.sample_s = 1.0 / self.sample_hz
        self.sample_rps = self.sample_hz * 2.0 * np.pi
        self.nyquist_rps = self.sample_rps / 2.0
        if tool_lag is None:
            min_possible_data_lag = 0.07 / self.nyquist_rps * data_lag_nyquist_ratio
            self.data_lag = min_possible_data_lag * 5.0
        ...
        index_start_sweep_lag = np.array(np.where(self.x_lag < -10.0 * std_dev_lag))[0, 0]
        time_start_sweep_lag = self.t[index_start_sweep_lag]
        index_end_sweep_lag = np.where(self.t < time_start_sweep_lag + 650.0)[0][-1]
        ...
        self.sampling_hz = [1.0 / self.dtime_zero_crossing[i] for i in range(len(self.dtime_zero_crossing))]
        ...
        plq(
            ...
            color="blue",
            linestyle="-",
            label="dtime_zero_crossing",
        )
        plt.text(
            0.5,
            0.2,
            f"tool_lag={self.data_lag} ",
    ...
    def lag(self):
        ...
        for i in range(self.N):
            lagged_val = self.ToolLag.calculate(
                self.x[i], reset=i < 1, dt=0.1
            )  # Actual dt req'd at run. 0.1 is placeholder
...
if __name__ == "__main__":
    ...
    """
    # Cx46000, new base 20251231
    data_file = (
        "C:/Users/daveg/Documents/GitHub/mySolarStateOfCharge/SOC_Particle/"
        "pyStateOfCharge/noise_study/sweepchirp4_soc2p2_hi_lo_chg.csv"
    )
    ...
    WaveVoVcn = Wave(mr.vovcn, mr.time, sample_hz=sample_hz, ss_time_rng=[10.0, 50.0])
