...
def compare_run():
    ...
    if modeling.get():
        ...
        compare_run_run(
            keys=keys, data_file_folder_run=Ref.version_path, data_file_folder_test=Test.version_path, terse=terse.get()
        )
...
def compare_run_sim_choose():
    ...
    if testpaths is None or testpaths == "":
        ...
        for testpath in testpaths:
            ...
            if key != "":
                compare_run_sim(
                    data_file=testpath, unit_key=key, strict_overplot=strict_overplot.get(), terse=terse.get()
                )
            else:
                tk.messagebox.showerror(message="key not found in" + testpath)
        update_data_buttons()
...
def save_putty():
    m_str = (
        datetime.datetime.fromtimestamp(Path(putty_test_csv_path.get()).stat().st_mtime)
...
def start_putty():
    ...
    if enter_size < 64:
        ...
        subprocess.Popen(
            ["putty", "-title", "putty-terminal-server", "-load", test_filename.get()],
...
if __name__ == "__main__":  # Example usage.  Ran ok 20260217
    ...
    if platform.system() == "Darwin":
        ...
        prev_button = myButton(
            option_panel_right,
            text="Run Prev",
            command=run_previous_task,
            fg="blue",
            bg=bg_color,
            justify="left",
            font=butt_font,
