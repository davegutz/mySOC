...
class LocalChem(Chemistry):
    def __init__(self, mod_code=0, rated_batt_cap=100.0, scale=1.0, dvoc=0.0):
        ...
        self.keep_sim_happy = (
            None  # Schedule -0.5 v so that sim shuts off lower than monitor because sim stops running when q=0
        )
    ...
    def assign_obs(self, keep_sim_happy=0.5):
        ...
        # Coulombic efficiency - the fraction of charging input that gets turned into usable Coulombs (.9976)
        self.coul_eff = 0.9976
        ...
        # Minimum temperature for valid saturation check, because BMS shuts off battery low. Heater should keep >4, too.
        # deg C (0.)
        self.low_t = 0.0
        ...
        self.t_soc = [0.47, 0.75, 0.80, 0.86]
        self.t_dv = [-0.10, -0.05, -0.04, 0.0, 0.02, 0.04, 0.05, 0.06, 0.07, 0.10]
    ...
    def __str__(self, prefix=""):
        ...
        s += (
            "\nIn app use \n#define NOM_UNIT_CAP        {:5.1f}   // Nominal battery unit capacity at RATED_TEMP."
            " (* 'Sc' or '*BS'/'*BP'), Ah\n"
...
if __name__ == "__main__":  # Example usage. Did not run on 20260217.  Useful for concepts only
    ...
    #  Instructions:
    #  Copy current values of obs battery from Chemistry_BMS for mod_code entered below in LocalChem, into assign_obs
    # above
    ...
    def main():
        ...
        obs = LocalChem(mod_code=1, rated_batt_cap=100.0, scale=1.05)  # rated_batt_cap/scale in Python
        obs.assign_all_mod()
