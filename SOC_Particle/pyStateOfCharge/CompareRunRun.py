...
def compare_pair_rms(run_path, ver_path, tol=1e-3, rtol=1e-3, tau=60.0):
    ...
    if n == 0:
        return {"file": run_path.name, "ver_file": ver_path.name, "error": "no rows with time >= 0", "diffs": []}
...
def report_rms(results, tol, rtol, tau):
    ...
    for r in results:
        ...
        if not r["diffs"]:
            print(
                f"  {pair_label}  — no EWMA differences > tol={tol}+rtol={rtol}*peak"
...
def plot_rms_diffs(results, data_file=None, save_plots=False, show_killer_=True, hardcopy=False):
    ...
    for r in results:
        ...
        for fig_idx in range(n_figs):
            ...
            fig, axes = plt.subplots(n_rows, _RMS_COLS, figsize=(5 * _RMS_COLS, 3 * n_rows), squeeze=False)
            fig_list.append(fig)
            fig_label = f"  [{fig_idx + 1}/{n_figs}]" if n_figs > 1 else ""
            fig.suptitle(f"EWMA diff  τ={tau}s\n{run_file}  vs  {ver_file}{fig_label}", fontsize=8)
            ...
            for sub_idx, d in enumerate(batch):
                ...
                ax.set_title(f"{d['param']}\npeak={d['peak_avg']:.4g}  thr=±{thr:.3g}", fontsize=8)
            ...
            fig_file_name = os.path.join(save_pdf_path, f"CompareRunRun_ewma_{len(fig_list)}.png")
...
def compare_run_run(
    ...
    fig_list = []
    fig_files = []
    dir_root_run, data_root_run = (
        str(PurePosixPath(data_file_run_clean).parent),
        PurePosixPath(data_file_run_clean).name,
    )
    data_root_run = data_root_run.replace(".csv", "")
    dir_root_test, data_root_test = (
        str(PurePosixPath(data_file_ver_clean).parent),
        PurePosixPath(data_file_ver_clean).name,
    )
    ...
    try:
        ...
        if _ewma_pairs:
            ...
            _rr_ret = plot_rms_diffs(
                _rr_results, data_file=data_file_test, save_plots=S.save_plots, show_killer_=False, hardcopy=hardcopy
            )
            if _rr_ret:
