...
#define ASK_DURING_BOOT \
  0  // Flag to ask for application of this file to * retained adjustments
     // 0=renom on dirty boot keep otherwise,1=ask
#define MODELING \
  0  // Nominal modeling bitmap (* 'Xm'), 0=all hdwe, 1+=Tb, 2+=Vb, 4+=Ib, 7=all
     // model.  +240 for discn

// Sensor biases
#define CURR_BIAS_AMP \
  0.00  // Calibration of amplified shunt sensor (* 'DA'), A, from 0.15 on
        // 20250608
...
#define IB_ABS_MAX_AMP \
  14.0  // Hard range limit of sensor electrically impossible (=1.65 *
        // SHUNT_GAIN * SHUNT_AMP_R1 / SHUNT_AMP_R2 *1.05) but saw -11.48 A (12)
#define IB_ABS_MAX_NOA \
  100.0  // Hard range limit of sensor electrically impossible (=1.65 *
         // SHUNT_GAIN * SHUNT_NOA_R1 / SHUNT_NOA_R2 *1.05) A (78.5)
#define HDWE_IB_HI_LO_NOA_LO \
  -11.  // Fully NOA bank discharge transition, A (-11)
#define HDWE_IB_HI_LO_AMP_LO \
  -10.  // Fully AMP bank discharge transition, A (-10)
...
// Battery.  One 12 V 100 Ah battery bank would have NOM_UNIT_CAP 100, NS 1, and
...
#define CHEM \
  0  // Chemistry monitor code integer, 0=Battleborn, 1=CHINS-guest room,
     // 2=CHINS-garage
#define NOM_UNIT_CAP \
  108.4  // Nominal battery unit capacity.  (* 'Sc' or ' BS'/' BP'), Ah logic, 1
         // = amp
...
// Faults
#define FAKE_FAULTS \
  false  // What to do with faults, T=detect and display them but don't change
         // signals
...
#define DISAB_TB_FA \
  true  // Had high out of range failure.  Want to keep running without latch on
        // the truck
...
#define IN_SERVICE \
  true  // Whether to put serial out BLE, which blocks and disrupts timed tests
