...
Iterator::Iterator(const String desc)
    : count_(0), desc_(desc), de_(0), des_(0), dx_(0), e_(0), ep_(0),
      limited_(false), x_(0), xmax_(0), xmin_(0), xp_(0) {}
Iterator::~Iterator() {}
// operators
// functions
...
// Inputs:  e_
// Outputs:  x_
double Iterator::iterate(const bool verbose, const uint16_t success_count,
  ...
  if (count_ == 2) {
    ...
    {
      no_soln = true;
      if (abs(ep_) < abs(e_)) x_ = xp_;
      ep_ = e_;
      limited_ = false;
      if (verbose)
        Serial.printf("%s:No soln\n",
                      desc_.c_str());  // Leaving x at most likely limit value
                                       // and recalculating...
      return e_;
    } else
      no_soln = false;
