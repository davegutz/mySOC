  ...
  *high = n - 1;
  ...
  /* Search if necessary  */
    ...
    *dx = (x - *(v + *low)) / (*(v + *high) - *(v + *low));
    if (sp.debug() == 93)
      Serial.printf(
          "binsearch: x %19.15f high %d low %d v[high] %19.15f v[low] %19.15f "
          "dx %19.15f\n",
...
double tab2(double x1, double x2, double* v1, double* v2, double* y, int n1,
  ...
  if (sp.debug() == 93)
    Serial.printf(
        "tab2: x %7.3f y %7.3f high1 %d high2 %d low1 %d low2 %d  temp1 %d "
        "temp2 %d dx1 %17.15f dx2 %17.15f r0 %17.15f r1 %17.15f result "
        "%19.15f\n",
        x1, x2, high1, high2, low1, low2, temp1, temp2, dx1, dx2, r0, r1,
        result);
  return result;
} /* End tab2 */
...
// class TableInterp
// constructors
TableInterp::TableInterp() : n1_(0) {}
TableInterp::TableInterp(const uint16_t n, double x[]) : n1_(n) {
  x_ = new double[n1_];
...
TableInterp::~TableInterp() { delete x_; }
// operators
// functions
...
TableInterp1D::~TableInterp1D() { delete v_; }
// operators
// functions
double TableInterp1D::interp(double x) { return tab1(x, x_, v_, n1_); }
...
TableInterp1Dclip::~TableInterp1Dclip() { delete v_; }
// operators
// functions
double TableInterp1Dclip::interp(double x) { return tab1(x, x_, v_, n1_); }
...
void TableInterp2D::pretty_print() {
  ...
  Serial.printf("      };\n");
  Serial.printf("    x={");
  for (i = 0; i < n1_; i++) Serial.printf("%7.3f, ", x_[i] - dx_);
  Serial.printf("};\n");
  Serial.printf("    y={");
  for (j = 0; j < n2_; j++) Serial.printf("%7.3f, ", y_[j] - dy_);
  Serial.printf("};\n");
}
